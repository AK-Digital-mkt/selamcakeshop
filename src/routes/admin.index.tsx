...
function AdminDashboard() {
  ...
  async function deleteOrder(id: string) {
    ...
    if (error) { alert("Delete failed: " + error.message); setOrders(prev); }
  }

  // Orders: bulk delete
  async function deleteSelectedOrders() {
    if (selectedOrderIds.length === 0) return;
    if (!confirm(`Delete ${selectedOrderIds.length} selected order(s) permanently? This cannot be undone.`)) return;
    const ids = [...selectedOrderIds];
    const prev = orders;
    setOrders((arr) => arr.filter((o) => !ids.includes(o.id)));
    setSelectedOrderIds([]);
    const { error } = await supabase.from("orders").delete().in("id", ids);
    if (error) { alert("Bulk delete failed: " + error.message); setOrders(prev); loadOrders(); }
  }

  // Orders: filter by created_at range
  const filteredOrders = orders.filter((o) => {
    const created = new Date(o.created_at).getTime();
    const now = Date.now();
    const day = 24 * 60 * 60 * 1000;
    const windowMs = orderRange === "daily" ? day : orderRange === "weekly" ? 7 * day : 30 * day;
            ...
            <section className="ma-card">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "inline-flex", background: "#F6D2E1", borderRadius: 12, padding: 4 }}>
                    {(["daily", "weekly", "monthly"] as const).map((r) => (
                      <button
                        ...
                        style={{
                          border: 0, cursor: "pointer", padding: "8px 16px", borderRadius: 10,
                          fontWeight: 700, fontSize: 13, textTransform: "capitalize",
                          background: orderRange === r ? "#88D4B0" : "transparent",
                          color: orderRange === r ? "#1F3A33" : "#4a5d54",
                  ...
                  {selectedOrderIds.length > 0 && (
                    <button
                      ...
                      type="button"
                      style={{ background: "#fee2e2", color: "#b91c1c", boxShadow: "none" }}
              ...
              <div className="ma-table-wrap">
                {filteredOrders.length === 0 ? (
                  <div className="ma-empty-state">No orders in this range.</div>
                ) : (
                  <table className="ma-table">
                    <thead>
                      <tr>
                        ...
                        <th>Order ID</th><th>Customer</th><th>Items</th><th>Delivery</th><th>Total</th><th>Placed</th><th>Status</th><th style={{ width: 60 }}></th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={o.id}>
                          ...
                          <td>
                            <button
                              ...
                              style={{
                                border: 0, cursor: "pointer", background: "#fee2e2", color: "#b91c1c",
                                width: 34, height: 34, borderRadius: 10, fontSize: 16, fontWeight: 700,
                              }}
                            >
                              🗑
                            </button>
...
function SettingsPanel() {
  ...
  const [splashMsg, setSplashMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);
  const splashFileRef = useRef<HTMLInputElement>(null);

  async function uploadSplashLogo(file: File) {
    setSplashMsg(null);
    const validationError = validateImageUpload(file);
    if (validationError) { setSplashMsg({ type: "err", text: validationError }); return; }
    setSplashUploading(true);
    try {
      const ext = getImageExtension(file);
      const path = `splash/logo-${Date.now()}.${ext}`;
      const { error: upErr } = await supabase.storage.from("cake-images").upload(path, file, {
        upsert: true, contentType: file.type || "image/jpeg",
      });
      if (upErr) { setSplashMsg({ type: "err", text: uploadFailureMessage(upErr.message) }); return; }
      const { data, error: sErr } = await supabase.storage.from("cake-images").createSignedUrl(path, 60 * 60 * 24 * 365 * 10);
      if (sErr || !data) { setSplashMsg({ type: "err", text: "The image uploaded, but the URL could not be created. Please try again." }); return; }
      const url = data.signedUrl;
      setSplashSaving(true);
      const { error } = await supabase
        .from("app_settings" as any)
        .upsert({ key: "splash_logo_url", value: url }, { onConflict: "key" });
      setSplashSaving(false);
      if (error) { setSplashMsg({ type: "err", text: "Could not save: " + error.message }); return; }
      setSplashLogoUrl(url);
      setSplashMsg({ type: "ok", text: "Startup logo updated successfully." });
    } catch (error) {
      setSplashMsg({ type: "err", text: uploadFailureMessage(error instanceof Error ? error.message : "Unexpected upload error") });
    } finally {
      setSplashUploading(false);
    }
  }

  async function resetSplashLogo() {
    setSplashMsg(null);
    setSplashSaving(true);
    const { error } = await supabase
      .from("app_settings" as any)
      .upsert({ key: "splash_logo_url", value: "" }, { onConflict: "key" });
    setSplashSaving(false);
    if (error) { setSplashMsg({ type: "err", text: "Could not reset: " + error.message }); return; }
    setSplashLogoUrl("");
    setSplashMsg({ type: "ok", text: "Startup logo reset to default." });
  }
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: splashData } = await supabase
        .from("app_settings" as any)
        .select("value")
        .eq("key", "splash_logo_url")
        .maybeSingle();
      setSplashLogoUrl(((splashData as any)?.value as string) ?? "");
    })();
  }, []);
  ...
  return (
    <>
      ...
      <section className="ma-card" style={{ marginBottom: 26 }}>
        <div className="ma-card-head">
          <h2 style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Camera size={18} /> Startup Logo
          </h2>
        </div>
        <div style={{ padding: 24, maxWidth: 520 }}>
          <p style={{ margin: "0 0 16px", color: "var(--ma-muted)", fontSize: 13.5 }}>
            Upload an image to show in the startup animation. Leave empty to use the default logo.
          </p>
          <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            {splashLogoUrl ? (
              <img src={splashLogoUrl} alt="Startup logo" style={{ width: 120, height: 120, objectFit: "contain", borderRadius: 12, background: "#fce6ee", padding: 8 }} />
            ) : (
              <div style={{ width: 120, height: 120, borderRadius: 12, background: "#f4f4f4", display: "grid", placeItems: "center", color: "var(--ma-muted)", fontSize: 12 }}>Default</div>
            )}
            <div>
              <label className="ma-upload">
                <Upload size={14} />
                {splashUploading || splashSaving ? "Uploading…" : "Upload Image"}
                <input
                  ref={splashFileRef}
                  type="file"
                  accept={ALLOWED_IMAGE_TYPES.join(",")}
                  disabled={splashUploading || splashSaving}
                  onChange={(e) => {
                    const f = e.target.files?.[0];
                    if (f) uploadSplashLogo(f);
              ...
              <p className="ma-upload-help">{IMAGE_UPLOAD_HELP}</p>
              {splashLogoUrl && (
                <button type="button" className="ma-btn-secondary" style={{ marginTop: 8, padding: "8px 14px" }} onClick={resetSplashLogo} disabled={splashSaving}>
                  Reset to default
                </button>
              )}
            </div>
